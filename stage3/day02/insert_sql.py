#-*-coding:utf-8-*-
from dbconn import  Department,Employees,Salary ,Session

session =Session()

# hr =Department(dep_id=1,dep_name='HR')   # 创建实例,一个实例一条记录
##session.add(hr)                          #将表记录添加到数据库
## session.commit()                        #确认提交
...
# qs = Department(dep_id=4, dep_name='测试部')
# fn = Department(dep_name='财务部')   # sqlalchemy把主键设置为了自动增长
# session.add_all([hr,ops, dev, qs, fn])        #批量添加记录
# session.commit()
